<template>
    <div class="toolball" :style="toolballStyle" @mouseenter="isExpanded = true" @mouseleave="isExpanded = false"
        @mousedown="startDrag">
        <!-- 主按钮 -->
        ...
        <transition name="panel-expand">
            <div v-if="isExpanded" class="tool-panel">
                <button v-for="(btn, i) in buttons" :key="btn.action" class="tool-button"
                    :style="{ '--delay-index': i }" @click="$emit(btn.action)" @mouseenter="btn.hover = true"
                    @mouseleave="btn.hover = false">
                    <img :src="btn.icon" :alt="btn.tooltip" class="icon-tool" :style="{
...
.tool-button {
    ...
    transition: all 0.3s cubic-bezier(0.18, 0.89, 0.32, 1.28) calc(var(--delay-index, 0) * 50ms);
}
